...
def get_preprocess_args():
    ...
    parser.add_argument('-n', '--name', default='len_for_bucket',
                        type=str, help='Name of the output directory', required=False)
...
def generate_length(args, tr_set, audio_extension):
    ...
    for i, s in enumerate(tr_set):
        ...
        # sort by len
        sorted_todo = [os.path.join(s, str(todo[idx]).split(
            s+'/')[-1]) for idx in reversed(np.argsort(tr_x))]
        # Dump data
        df = pd.DataFrame(data={'file_path': [fp for fp in sorted_todo], 'length': list(
            reversed(sorted(tr_x))), 'label': None})
...
def main():
    ...
    if 'librilight' in args.input_data.lower():
        SETS = ['small', 'medium', 'large'] + \
            ['small-splitted', 'medium-splitted', 'large-splitted']
