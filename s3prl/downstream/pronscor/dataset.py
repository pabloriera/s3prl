...
class PronscorDataset(Dataset):
    ...
    def __init__(self, split, bucket_size, data_root, phone_path, alignment_file, train_split_file, test_split_file, bucket_file, sample_rate=16000, train_dev_seed=1337, **kwargs):
        ...
        phone_file = open(os.path.join(
            phone_path, alignment_file)).readlines()
        ...
        if split == 'train':
            train_list = open(os.path.join(
                phone_path, train_split_file)).readlines()

            usage_list = [line.strip('\n') for line in train_list]
        elif split == 'dev' or split == 'test':
            test_list = open(os.path.join(
                phone_path, test_split_file)).readlines()
